...
def job_search(username):
    ...
    deleted_applied_jobs = [
        job for job in applied_jobs if job not in get_all_job_titles()
    ]
    if deleted_applied_jobs:
        print("Here are the jobs you applied for, but got deleted:")
    ...
    # Else if feature d is chosen, then prompt to go back to feature select
    elif feature_choice == "d":
        job_delete(username)
