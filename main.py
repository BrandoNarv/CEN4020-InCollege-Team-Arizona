...
def signup():
  ...
  print("""Password must be 8-12 characters long and contain at least \
  ...
  account_type = (input(
      "Would you like to upgrade your account? (Y / N): ").strip().upper())
...
def validate_password(input_p):
  ...
  if not (8 <= len(input_p) <= 12 and any(char.isupper() for char in input_p)
          and any(char.isdigit()
                  for char in input_p) and any(char in string.punctuation
                                               for char in input_p)):
...
def job_delete(username):
  ...
  jobs = get_job_list_posted_by_user(get_first_name(username),
                                     get_last_name(username))
  if jobs:
  ...
  if jobs:
    delete_job_confirmation = input(
        "Would you like to delete a job? y/n: ").lower()
    if delete_job_confirmation == "y":
      delete_job_title = input(
          "Please enter the job title you want to delete: ")
...
def save_job(username):
  ...
  jobs = [
      unsaved_job for unsaved_job in all_jobs_list(username)
  ...
  if unsave_job_confirmation == "y":
    ...
    if saved_jobs:
      ...
      for idx, job in enumerate(saved_jobs):
        print(f"{idx + 1} - {job}")
      print("\n")
      unsave_job_title = input(
          "Please enter the job title you want to unsave: ")
...
def job_listing(username):
  ...
  for key, value in JOB_LISTING.items():
    print(f"{key}. {value}")
  feature_choice = input(
      f"Choose one of {list(JOB_LISTING.keys())}:").strip().lower()
...
def list_all_jobs(username):
  ...
  if all_jobs:
    ...
    view_info = input(
        "Do you want to apply to any of the jobs on this list? y/n?: ").lower(
        )
...
def apply_for_job(username):
  ...
  if job_info:
    ...
    confirm_apply = input(
        "Confirm this job and send the application? y/n?: ").lower()
...
def send_application(username, job_title):
  ...
  origin_check = user_made_job(get_first_name(username),
                               get_last_name(username), job_title)
  ...
  # If the user has not applied to this job, and they don't own it, then send the application
    ...
    print("\n")
    graduation = input(
        "Please enter your predicted graduation date(mm/dd/yyyy): ")
    ...
    print("\n")
    description = input(
        "Please explain why you'd be a great fit for this job: ")

    # Take user's information and save as an application
    print("\n")
    verify_apply = create_application(username, job_title, graduation, start,
                                      description)
...
def choose_navigation_link():
  ...
  navigation_link_choice = (
      input(f"Choose one of {list(NAVIGATION_LINKS_GROUP.keys())}: ").strip(
      ).lower())
...
def choose_useful_links():
  ...
  # Prompt the user to choose a link
  useful_link_choice = (input(
      f"Choose one of {list(USEFUL_LINKS_GROUP.keys())}: ").strip().lower())
...
def choose_incollege_important_links():
  ...
  if important_link_choice in INCOLLEGE_IMPORTANT_LINKS_GROUP:
    print(
        f"You selected {INCOLLEGE_IMPORTANT_LINKS_GROUP[important_link_choice]}"
    )
...
def privacy_policy():
  ...
  option = input(
      f"Choose one of {list(GUEST_CONTROLS.keys())}:").strip().lower()
...
def languages():
  ...
  if signed_in:
    ...
    language = LANGUAGES[input(
        f"Choose one of {list(LANGUAGES.keys())}:").strip().lower()]
...
def general():
  ...
  if signed_in == False:
    for key, value in NOT_SIGNED_IN_GENERAL_LINKS_GROUP.items():
      print(f"{key}. {value}")
    general_link_choice = (input(
        f"Choose one of {list(NOT_SIGNED_IN_GENERAL_LINKS_GROUP.keys())}: ").
                           strip().lower())
  ...
  # Else if you are signed in, display the signed in general choices
    ...
    if general_link_choice in SIGNED_IN_GENERAL_LINKS_GROUP:
      print(
          f"You selected {SIGNED_IN_GENERAL_LINKS_GROUP[general_link_choice]}")
...
def display_choice(username):
  ...
  while True:
    ...
    user_input = (
        input("Do you want to see what your profile looks like (Y/N)? ").strip(
        ).upper())
